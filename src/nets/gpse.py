...
class GNNInductiveHybridMultiHead(torch.nn.Module):
    ...
    def _pad_and_stack(self, x1: torch.Tensor, x2: torch.Tensor, pad1: int, pad2: int):
        padded_x1 = nn.functional.pad(x1, (0, pad2))
        padded_x2 = nn.functional.pad(x2, (pad1, 0))
        return torch.vstack([padded_x1, padded_x2])

    def _apply_index(self, batch, virtual_node: bool, pad_node: int, pad_graph: int):
    ...
    def forward(self, batch):
        batch.node_feature = torch.hstack([m(batch.x) for m in self.node_post_mps])
        graph_emb = self.graph_pooling(batch.x, batch.batch)
        batch.graph_feature = self.graph_post_mp(graph_emb)
        return self._apply_index(
            batch, self.virtual_node, self.node_target_dim, self.graph_target_dim
        )
...
class GPSE(torch.nn.Module):
    ...
    url_dict = {
        ...
        "pcqm4mv2": "https://zenodo.org/record/8145095/files/"
    ...
    def from_pretrained(cls, name: str, root: str = "GPSE_pretrained"):
        ...
        model_state_new = OrderedDict(
            [(k.split(".", 1)[1], v) for k, v in model_state.items()]
        )
...
class GPSENodeEncoder(torch.nn.Module):
    ...
    def __init__(
        ...
        assert dim_emb > dim_pe_out, (
            "Desired GPSE dimension (dim_pe_out) "
        ...
        if model_type == "mlp":
            ...
            if n_layers == 1:
                ...
                for _ in range(n_layers - 2):
                    layers.append(torch.nn.Linear(2 * dim_pe_out, 2 * dim_pe_out))
                    layers.append(activation())
    ...
    **kwargs,
    ...
    if neighbor_loader:
        if layers_mp is None:
            raise ValueError(
                'Please provide the number of message-passing layers as "layers_mp".'
            )
        ...
        loader = NeighborLoader(
            data, num_neighbors=num_neighbors, shuffle=False, pin_memory=True, **kwargs
        )
        out_list = []
...
def gpse_process_batch(
    ...
    **kwargs,
    ...
    if neighbor_loader:
        ...
        loader = NeighborLoader(
            batch, num_neighbors=num_neighbors, shuffle=False, pin_memory=True, **kwargs
        )
        out_list = []
...
def precompute_GPSE(
    ...
    **kwargs,
    ...
    for batch in loader:
        batch_out, batch_ptr = gpse_process_batch(model, batch, rand_type, **kwargs)
...
def cosim_col_sep(
    pred: torch.Tensor, true: torch.Tensor, batch_idx: torch.Tensor
) -> torch.Tensor:
    ...
    if batch_idx is None:
        raise ValueError(
            "mae_cosim_col_sep requires batch index as "
...
def gpse_loss(
    pred: torch.Tensor, true: torch.Tensor, batch_idx: torch.Tensor = None
) -> Tuple[torch.Tensor, torch.Tensor]:
...
class AddGPSE(BaseTransform):
    ...
    def __init__(self, model: Module, use_vn: bool = True, rand_type: str = "NormalSE"):
        self.model = model
        self.use_vn = use_vn
        self.vn = VirtualNode()
