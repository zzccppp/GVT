...
class GPT(nn.Module):
    ...
    def forward(self, idx, targets=None, return_last_state=False):
        device = idx.device
        b, t = idx.size()
...
class GPTPermInv(GPT):
    def forward(self, idx, targets=None, return_last_state=False):
        ...
        # 首先计算累加和
        cumsum = torch.cumsum(logits, dim=1)
        # 创建位置索引，用于除以对应的位置数
        positions = torch.arange(1, seq_length + 1, device=logits.device).view(1, -1, 1)
        # 计算累加平均值 (cumsum / positions)
        cumulative_avg = cumsum / positions

        logits = cumulative_avg
